...
{
    public partial class frmUrunAramaListe1 : Form
    {
        ...
        private void frmUrunAramaListe1_Load(object sender, EventArgs e)
        {  VeriAl(); }
        ...
        {   try
            {   con = new OleDbConnection(cs);
                con.Open();
                cmd = new OleDbCommand("SELECT (UrunAd) as [Ürün Adı],Kategori,Marka from Urun where Marka like '" + txtMarka.Text + "%' order by UrunAd", con);
                rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection);
                dataGridView1.Rows.Clear();
                while (rdr.Read() == true)
                { dataGridView1.Rows.Add(rdr[0], rdr[1], rdr[2]); }
        ...
        private void frmUrunAramaListe1_FormClosing(object sender, FormClosingEventArgs e)
        {
            this.Hide();
        ...
        {   try
            {   DataGridViewRow dr = dataGridView1.SelectedRows[0];
                ...
                frmUrun frm = new frmUrun();
                frm.Show();
                frm.txtÜrünAd.Text = dr.Cells[0].Value.ToString();
                frm.textBox1.Text = dr.Cells[0].Value.ToString();
                frm.cmbKategori.Text = dr.Cells[1].Value.ToString();
                frm.cmbMarka.Text = dr.Cells[2].Value.ToString();
